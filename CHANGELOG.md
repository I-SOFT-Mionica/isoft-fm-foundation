...
- **Arbiter addon announcement.** New "I-Soft File Manager: Foundation Arbiter" entry on the Extensions tab (alongside Sentinel + Orbit) describing the planned one-shot jDownloads importer. The readme FAQ that previously claimed "a migration tool is planned but not yet shipped" now points at Arbiter explicitly. Splitting the importer into its own plugin keeps Foundation core lean (jDownloads-aware code only runs when Arbiter is installed) and gives the importer its own WP.org release cycle.
