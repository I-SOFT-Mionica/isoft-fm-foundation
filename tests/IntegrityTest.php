<?php
/**
 * ISOFT_FMF_File_Integrity tests — missing detection, inode relink, auto-republish.
 *
 * These tests create real files under isoft_fmf_files_dir() so inode/hash behavior is
 * exercised end-to-end. POSIX inode recovery is skipped on Windows runners.
 */

class IntegrityTest extends WP_UnitTestCase {

	private ISOFT_FMF_File_Integrity $integrity;
	...
	public function set_up(): void {
		parent::set_up();
		$this->integrity = new ISOFT_FMF_File_Integrity();

		// Create category + its folder.
		$term           = wp_insert_term( 'Integrity Cat', 'isoft_fmf_category', array( 'slug' => 'integrity-cat' ) );
		$this->term_id  = (int) $term['term_id'];
		$this->category_dir = isoft_fmf_category_fs_path( $this->term_id );
		if ( ! is_dir( $this->category_dir ) ) {
			wp_mkdir_p( $this->category_dir );
		}

		$this->download_id = (int) isoft_fmf_create_draft_download( array( 'title' => 'Integrity Host' ) );
		wp_set_object_terms( $this->download_id, array( $this->term_id ), 'isoft_fmf_category' );
		wp_update_post(
			array(
				'ID'          => $this->download_id,
				'post_status' => 'publish',
		...
		update_option( 'isoft_fmf_integrity_use_inode', 1 );
		update_option( 'isoft_fmf_integrity_autorelink', 1 );
	}
	...
	/** Create a real file on disk and a matching wp_isoft_fmf_files row; returns row id. */
	private function make_local_file( string $name, string $contents ): int {
		global $wpdb;
		$abs      = $this->category_dir . '/' . $name;
		file_put_contents( $abs, $contents );
		$rel      = ltrim( str_replace( isoft_fmf_files_dir(), '', $abs ), '/\\' );
		$rel      = str_replace( '\\', '/', $rel );
	...
	private function get_row( int $id ): object {
		global $wpdb;
		return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}isoft_fmf_files WHERE id = %d", $id ) );
	}
	...
	public function test_handle_missing_marks_row_and_sets_timestamp(): void {
		...
		$mode = ISOFT_FMF_File_Integrity::handle_missing( $row, $this->download_id );
	...
	public function test_handle_missing_is_idempotent(): void {
		...
		ISOFT_FMF_File_Integrity::handle_missing( $row, $this->download_id );
		$first_notices = count( (array) get_option( 'isoft_fmf_admin_notices', array() ) );

		// Second call must not re-queue (pass the post-update row, which now has is_missing=1).
		$row2 = $this->get_row( $id );
		ISOFT_FMF_File_Integrity::handle_missing( $row2, $this->download_id );
		$second_notices = count( (array) get_option( 'isoft_fmf_admin_notices', array() ) );
	...
	public function test_handle_missing_unpublishes_when_all_files_missing(): void {
		...
		$this->assertNotEmpty( get_post_meta( $this->download_id, '_isoft_fmf_auto_unpublished_at', true ) );
	}
	...
	public function test_handle_missing_does_not_unpublish_when_other_files_healthy(): void {
		...
		$mode = ISOFT_FMF_File_Integrity::handle_missing( $row1, $this->download_id );
		$this->assertSame( 'partial', $mode );
		$this->assertSame( 'publish', get_post_status( $this->download_id ) );
		$this->assertEmpty( get_post_meta( $this->download_id, '_isoft_fmf_auto_unpublished_at', true ) );
	}
	...
	public function test_try_relink_returns_false_when_inode_absent(): void {
		...
		global $wpdb;
		$wpdb->update( $wpdb->prefix . 'isoft_fmf_files', array( 'inode' => null ), array( 'id' => $id ), array( '%d' ), array( '%d' ) );
	...
	public function test_scheduled_check_reschedules_on_option_change(): void {
		update_option( 'isoft_fmf_integrity_check_enabled', 1 );
		update_option( 'isoft_fmf_integrity_check_time', '03:15' );
		$this->integrity->maybe_schedule();
		$this->assertNotFalse( wp_next_scheduled( 'isoft_fmf_integrity_check' ) );

		update_option( 'isoft_fmf_integrity_check_enabled', 0 );
		$this->integrity->reschedule();
		$this->assertFalse( wp_next_scheduled( 'isoft_fmf_integrity_check' ) );
	}
	...
	public function test_auto_republish_only_fires_when_auto_unpublished_flag_set(): void {
		...
		$manual = (int) isoft_fmf_create_draft_download( array( 'title' => 'Manual Draft' ) );
		wp_set_object_terms( $manual, array( $this->term_id ), 'isoft_fmf_category' );
		...
		// No _isoft_fmf_auto_unpublished_at meta — run scan, should heal but not publish.
		$this->integrity->run_scheduled_check();
